...
CONTAINS
  ...
  !! Compute the sum of the end position of the walkers after a time T.
  !! eps is the set of all random numbers needed to do the simulation.
  !! For each walker k:
  !!   - eps(1+2*(k-1)*nsteps:(2*k-1)*nsteps) follow a uniform distribution between 0 and 1 (to decide whether walkers die).
  !!   - eps((2*k-1)*nsteps+1:2*k*nsteps) follow a Gaussian distribution of zero mean and variance 2. * dt (random kicks).
  FUNCTION compute_end_position(eps, nwalker, nsteps, dt, T) RESULT(xf)
    INTEGER, INTENT(IN)        :: nsteps, nwalker
    REAL(C_DOUBLE), INTENT(IN) :: eps(:), dt, T
    REAL(C_DOUBLE)             :: xf, death_rate, pos(nwalker)
    INTEGER                    :: i, nalive, k, ind
    ...
    DO i = 1, nsteps
       ...
       DO k = 1, nwalker
          IF (alive(k)) THEN
             ind = i + 2 * ( k - 1 ) * nsteps
             IF (eps(ind) < death_rate) THEN
                alive(k) = .FALSE.
                nalive = nalive - 1
                death_rate = nalive * dt
             ELSE
                pos(k) = pos(k) + eps(ind + nsteps)
             END IF
          END IF
       END DO
    END DO
    xf = SUM(pos) / T
  END FUNCTION compute_end_position
    
  !! Create the entire set eps of random numbers.
  !! For each walker k:
  !!   - eps(1+2*(k-1)*nsteps:(2*k-1)*nsteps) follow a uniform distribution between 0 and 1 (to decide whether walkers die).
  !!   - eps((2*k-1)*nsteps+1:2*k*nsteps) follow a Gaussian distribution of zero mean and variance 2. * dt (random kicks).
  SUBROUTINE create_random_numbers(VectorState, nwalker, nsteps, noise_amp, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    INTEGER, INTENT(IN)               :: nsteps, nwalker
    REAL(C_DOUBLE), INTENT(IN)        :: noise_amp
    REAL(C_DOUBLE), ALLOCATABLE       :: eps(:)
    INTEGER                           :: i, k, ind
    IF (.NOT.ALLOCATED(eps)) ALLOCATE(eps(2 * nwalker * nsteps))
    DO i = 1, nsteps
       ind = i
       DO k = 1, nwalker
          eps(ind) = random(VectorState)
          ind = ind + nsteps
          eps(ind) = random_gaussian(VectorState) * noise_amp
          ind = ind + nsteps
       END DO
    END DO
  ...
  !! For n_update instants, the two random numbers (uniform and Gaussian) of all walkers are updated.
  SUBROUTINE update_random_numbers(VectorState, nwalker, nsteps, noise_amp, n_update, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
  ...
  SUBROUTINE input(input_cnf, file_output, file_histo, file_final_state, dt, T, nwalker, &
                   noise_amp, nsteps, ntrials, nsamples, n_update, kappa, x0, xmin, xmax, &
                   dx, in_SS)
    CHARACTER(256), INTENT(OUT)     :: input_cnf, file_output, file_final_state, file_histo
    INTEGER, INTENT(OUT)            :: nsteps, nsamples, n_update, nwalker
    INTEGER(C_INT64_T), INTENT(OUT) :: ntrials
    LOGICAL, INTENT(OUT)            :: in_SS
    REAL(C_DOUBLE), INTENT(OUT)     :: dt, T, noise_amp
    ...
    DO
       ...
       IF (TRIM(param) == "nwalker") READ(value_param(1:LEN_TRIM(value_param)),*) nwalker !! number of mortal walkers to simulate
    END DO
10  CONTINUE
    CLOSE(UNIT = UNIT)
...
17  CONTINUE
    ...
    END IF
    IF (nwalker < 1) THEN
       WRITE(*, "(a)") "The number of walkers must be positive."
       STOP
    END IF
  ...
  SUBROUTINE read_init_state(UNIT, nwalker, nsteps, eps)
    INTEGER, INTENT(IN)         :: UNIT, nsteps, nwalker
    REAL(C_DOUBLE), ALLOCATABLE :: eps(:)
    IF (.NOT.ALLOCATED(eps)) ALLOCATE(eps(2 * nwalker * nsteps))
    READ(UNIT, *) eps(:)
...
PROGRAM main
  ...
  INTEGER                         :: nsteps, n_update, nwalker
  ...
  CALL input(file_init, file_output, file_histo, file_final_state, dt, T, nwalker, noise_amp, &
  ...
  ELSE
     CALL create_random_numbers(VectorState, nwalker, nsteps, noise_amp, eps)
  END IF
  ...
  x = compute_end_position(eps, nwalker, nsteps, dt, T)
  ALLOCATE(eps_trial(SIZE(eps)))
  CALL CPU_TIME(start_time)
  DO i = 1_C_INT64_T, ntrials
     eps_trial = eps
     CALL update_random_numbers(VectorState, nwalker, nsteps, noise_amp, n_update, eps_trial)
     trial_x = compute_end_position(eps_trial, nwalker, nsteps, dt, T)
