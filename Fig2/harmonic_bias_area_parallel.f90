...
CONTAINS
  ...
  !! Compute the area under the trajectory of the walker up to time T until it dies.
  ...
  FUNCTION compute_area(eps, nsteps, dt, Tsq) RESULT(area)
    INTEGER, INTENT(IN)        :: nsteps
    REAL(C_DOUBLE), INTENT(IN) :: eps(:), dt, Tsq
    REAL(C_DOUBLE)             :: area
    INTEGER                    :: stop_index, i, weight
    stop_index = nsteps
    DO i = 1, nsteps
        IF (eps(i) < dt) THEN
            stop_index = i - 1
  ...
  SUBROUTINE input(input_cnf, file_output, file_histo, file_final_state, dt, Tsq, noise_amp, &
    ...
    REAL(C_DOUBLE), INTENT(OUT)     :: dt, Tsq, noise_amp
...
PROGRAM main
  ...
  REAL(C_DOUBLE)                  :: dt, Tsq, noise_amp
  ...
  CALL input(file_init, file_output, file_histo, file_final_state, dt, Tsq, noise_amp, nsteps, &
  ...
  x = compute_area(eps, nsteps, dt, Tsq)
  ...
  DO i = 1_C_INT64_T, ntrials
     ...
     trial_x = compute_area(eps_trial, nsteps, dt, Tsq)
