...
CONTAINS
  ...
  !! Compute the residence time of the walker during the entire simulation or 
  !! before it gets absorbed by the boundary between xl and xr.
  ...
  FUNCTION compute_residence_time(eps, nsteps, dt, T, xl, xr) RESULT(res_time)
    INTEGER, INTENT(IN)        :: nsteps
    REAL(C_DOUBLE), INTENT(IN) :: eps(:), dt, T, xl, xr
    REAL(C_DOUBLE)             :: res_time, pos, pos_before
    INTEGER                    :: i
    pos = 0.5_C_DOUBLE ! Start from the middle of the interval
    res_time = 0.0_C_DOUBLE
    DO i = 1, nsteps
       pos_before = pos
       pos = pos + eps(i)
       ...
       END IF
       IF ( (pos .GE. xl) .AND. (pos .LE. xr) ) THEN 
          res_time = res_time + dt
       END IF
       IF (pos <= 0.0_C_DOUBLE) THEN
          pos = 0.0_C_DOUBLE
          EXIT
       END IF
       IF (pos * pos_before < eps(nsteps + i)) THEN
          pos = 0.0_C_DOUBLE
          EXIT
       END IF
    END DO
    res_time = res_time / T
  END FUNCTION compute_residence_time
  ...
  !! eps(nsteps+1:2*nsteps) follow an exponential distribution of parameter 1 / dt.
  SUBROUTINE create_random_numbers(VectorState, nsteps, dt, noise_amp, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    INTEGER, INTENT(IN)               :: nsteps
    REAL(C_DOUBLE), INTENT(IN)        :: dt, noise_amp
    REAL(C_DOUBLE), ALLOCATABLE       :: eps(:)
    INTEGER                           :: i
    IF (.NOT.ALLOCATED(eps)) ALLOCATE(eps(nsteps + nsteps))
    DO i = 1, nsteps
       eps(i) = random_gaussian(VectorState) * noise_amp
       eps(i + nsteps) = - dt * LOG(1.0_C_DOUBLE - random(VectorState))
    END DO
  END SUBROUTINE create_random_numbers
  
  !! Update the array eps of random numbers.
  !! For n_update instants, the two random numbers (Gaussian and exponential) are updated.
  SUBROUTINE update_random_numbers(VectorState, nsteps, dt, noise_amp, n_update, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    ...
    DO WHILE (counter < n_update)
       ...
       IF (chosen(trial_ind) == 0) THEN
          ...
          eps(trial_ind + nsteps) = - dt * LOG(1.0_C_DOUBLE - random(VectorState))
       END IF
    END DO
  END SUBROUTINE update_random_numbers

  !! Read simulation parameters from input files provided on the command line.
  SUBROUTINE input(input_cnf, file_output, file_histo, file_final_state, dt, T, noise_amp, xl, &
                   xr, nsteps, ntrials, nsamples, n_update, kappa, x0, xmin, xmax, dx, in_SS)
    ...
    REAL(C_DOUBLE), INTENT(OUT)     :: dt, T, noise_amp, xl, xr
    ...
    DO
       ...
       IF (TRIM(param) == "left_boundary") READ(value_param(1:LEN_TRIM(value_param)),*) xl !! left boundary of the interval to define the residence time
       IF (TRIM(param) == "right_boundary") READ(value_param(1:LEN_TRIM(value_param)),*) xr !! right boundary of the interval to define the residence time
    END DO
10  CONTINUE
    CLOSE(UNIT = UNIT)
...
17  CONTINUE
    ...
    END IF
    IF (xl > xr) THEN
       WRITE(*, "(a)") "The interval for the residence time is not provided."
       STOP
    END IF
...
PROGRAM main
  ...
  REAL(C_DOUBLE)                  :: dt, T, noise_amp, xl, xr
  ...
  CALL input(file_init, file_output, file_histo, file_final_state, dt, T, noise_amp, xl, &
             xr, nsteps, ntrials, nsamples, n_update, kappa, x0, xmin, xmax, dx, in_SS)  
  ...
  ELSE
     CALL create_random_numbers(VectorState, nsteps, dt, noise_amp, eps)
  END IF
  ...
  x = compute_residence_time(eps, nsteps, dt, T, xl, xr)
  ALLOCATE(eps_trial(SIZE(eps)))
  CALL CPU_TIME(start_time)
  DO i = 1_C_INT64_T, ntrials
     eps_trial = eps
     CALL update_random_numbers(VectorState, nsteps, dt, noise_amp, n_update, eps_trial)
     trial_x = compute_residence_time(eps_trial, nsteps, dt, T, xl, xr)
