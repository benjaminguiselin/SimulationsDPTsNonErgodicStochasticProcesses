...
CONTAINS
  ...
  !! Compute the end position of the walker after a time T and returns the final position.
  ...
  FUNCTION compute_end_position(eps, nsteps, dt, T) RESULT(xf)
    ...
    DO i = 1, nsteps
        ...
        END IF
    END DO
    xf = SUM(eps(nsteps + 1:nsteps + stop_index)) / T
...
10  CONTINUE
    ...
    DO
       ...
       IF (TRIM(param) == "x0") READ(value_param(1:LEN_TRIM(value_param)),*) x0 !! centre of the harmonic bias
...
PROGRAM main
  ...
  CALL input(file_init, file_output, file_histo, file_final_state, dt, T, noise_amp, nsteps, &
  ...
  x = compute_end_position(eps, nsteps, dt, T)
  ...
  DO i = 1_C_INT64_T, ntrials
     ...
     trial_x = compute_end_position(eps_trial, nsteps, dt, T)
