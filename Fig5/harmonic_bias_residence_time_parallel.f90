...
CONTAINS
  ...
  !! Compute the residence time of the walker up to time T until it dies between xl and xr.
  !! eps is the set of all random numbers needed to do the simulation: 
  !!   - eps(1:nsteps) follow a uniform distribution between 0 and 1 (decide whether the walker dies).
  !!   - eps(nsteps + 1:2 * nsteps) follow a Gaussian distribution of zero mean and variance 2. * dt (random kicks).
  FUNCTION compute_residence_time(eps, nsteps, dt, T, xl, xr) RESULT(res_time)
    INTEGER, INTENT(IN)        :: nsteps
    REAL(C_DOUBLE), INTENT(IN) :: eps(:), dt, T, xl, xr
    REAL(C_DOUBLE)             :: res_time, pos
    INTEGER                    :: stop_index, i
    stop_index = nsteps
    DO i = 1, nsteps
        IF (eps(i) < dt) THEN
            stop_index = i - 1
  ...
  !!   - eps(1:nsteps) follow a uniform distribution between 0 and 1 (decide whether the walker dies).
  !!   - eps(nsteps + 1:2 * nsteps) follow a Gaussian distribution of zero mean and variance 2. * dt (random kicks).
  SUBROUTINE create_random_numbers(VectorState, nsteps, noise_amp, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    INTEGER, INTENT(IN)               :: nsteps
    ...
    END DO
    DO i = nsteps + 1, 2 * nsteps
       eps(i) = random_gaussian(VectorState) * noise_amp
    END DO
  END SUBROUTINE create_random_numbers
  
  !! Update the array eps of random numbers.
  !! For n_update instants, the two random numbers (uniform and Gaussian) are updated.
  SUBROUTINE update_random_numbers(VectorState, nsteps, noise_amp, n_update, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    ...
    DO WHILE (counter < n_update)
       ...
       IF (chosen(trial_ind) == 0) THEN
          chosen(trial_ind) = 1
          counter = counter + 1
          eps(trial_ind) = random(VectorState)
          eps(trial_ind + nsteps) = random_gaussian(VectorState) * noise_amp
       END IF
    END DO
  END SUBROUTINE update_random_numbers

  !! Read simulation parameters from input files provided on the command line.
  SUBROUTINE input(input_cnf, file_output, file_histo, file_final_state, dt, T, noise_amp, &
                   xl, xr, nsteps, ntrials, nsamples, n_update, kappa, x0, xmin, xmax, dx, &
                   in_SS)
...
17  CONTINUE
    CLOSE(UNIT = UNIT)
    noise_amp = SQRT(2.0_C_DOUBLE * dt) !! standard deviation of the Gaussian random numbers (walker kicks)
    T = dt * nsteps
  ...
  SUBROUTINE read_init_state(UNIT, nsteps, eps)
    ...
    REAL(C_DOUBLE), ALLOCATABLE :: eps(:)
    IF (.NOT.ALLOCATED(eps)) ALLOCATE(eps(2 * nsteps))
    READ(UNIT, *) eps(:)
...
PROGRAM main
  ...
  ELSE
     CALL create_random_numbers(VectorState, nsteps, noise_amp, eps)
  END IF
  ...
  DO i = 1_C_INT64_T, ntrials
     eps_trial = eps
     CALL update_random_numbers(VectorState, nsteps, noise_amp, n_update, eps_trial)
