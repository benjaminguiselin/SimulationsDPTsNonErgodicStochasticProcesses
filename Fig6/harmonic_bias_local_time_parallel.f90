...
CONTAINS
  ...
  !! We use the polar method of Marsaglia and Bray.
  FUNCTION random_gaussian(VectorState) RESULT(xi)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    REAL(C_DOUBLE)                    :: xi, xi1, xi2, r2
    r2 = 2.0_C_DOUBLE
    DO WHILE ((r2 .GE. 1.0_C_DOUBLE) .OR. (r2 .EQ. 0.0_C_DOUBLE))
       xi1 = 2.0_C_DOUBLE * random(VectorState) - 1.0_C_DOUBLE
       xi2 = 2.0_C_DOUBLE * random(VectorState) - 1.0_C_DOUBLE
       r2 = xi1 * xi1 + xi2 * xi2
    END DO
    xi = SQRT(- 2.0_C_DOUBLE * LOG(r2) / r2) * xi1
  END FUNCTION random_gaussian

  !! Compute the local time at position xa of the walker during the entire 
  !! simulation or before it gets absorbed by the boundary.
  !! eps is the set of all random numbers needed to do the simulation.
  !! eps(1:nsteps) are Gaussian random numbers of zero mean and variance 2. * dt.
  !! eps(nsteps+1:2*nsteps) follow an exponential distribution of parameter 1 / dt.
  !! eps(2*nsteps+1:3*nsteps) follow an exponential distribution of parameter 1 / dt.
  !! eps(3*nsteps+1:4*nsteps) follow an exponential distribution of parameter 1. / (4. * dt).
  FUNCTION compute_local_time(eps, nsteps, T, xa) RESULT(local_time)
    INTEGER, INTENT(IN)        :: nsteps
    REAL(C_DOUBLE), INTENT(IN) :: eps(:), T, xa
    REAL(C_DOUBLE)             :: local_time, pos, pos_before, dx, dx_before, tmp
    INTEGER                    :: i, ind
    pos = 0.75_C_DOUBLE ! Start from x0=3L/4
    local_time = 0.0_C_DOUBLE
    DO i = 1, nsteps
       pos_before = pos
       pos = pos + eps(i)
       IF (pos > 1.0_C_DOUBLE) THEN
          pos = 2.0_C_DOUBLE - pos
       END IF
       IF (pos <= 0.0_C_DOUBLE) THEN
          pos = 0.0_C_DOUBLE
          EXIT
       END IF
       ind = i + nsteps
       IF (pos * pos_before < eps(ind)) THEN
          pos = 0.0_C_DOUBLE
          EXIT
       END IF
       ind = ind + nsteps
       dx = pos - xa
       dx_before = pos_before - xa
       IF (dx * dx_before < eps(ind)) THEN
          ind = ind + nsteps
          tmp = ABS(dx) + ABS(dx_before)
          local_time = local_time + SQRT(tmp * tmp + eps(ind)) - tmp
       END IF
    END DO
    local_time = local_time * 0.5_C_DOUBLE / T
  END FUNCTION compute_local_time
    
  !! Create the entire set eps of random numbers needed for the simulation.
  !! eps(1:nsteps) follow a Gaussian distribution of zero mean and variance 2. * dt.
  !! eps(nsteps+1:2*nsteps) follow an exponential distribution of parameter 1 / dt.
  !! eps(2*nsteps+1:3*nsteps) follow an exponential distribution of parameter 1 / dt.
  !! eps(3*nsteps+1:4*nsteps) follow an exponential distribution of parameter 1. / (4. * dt).
  SUBROUTINE create_random_numbers(VectorState, nsteps, dt, four_dt, noise_amp, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    INTEGER, INTENT(IN)               :: nsteps
    REAL(C_DOUBLE), INTENT(IN)        :: dt, four_dt, noise_amp
    REAL(C_DOUBLE), ALLOCATABLE       :: eps(:)
    INTEGER                           :: i, j
    IF (.NOT.ALLOCATED(eps)) ALLOCATE(eps(4 * nsteps))
    DO i = 1, nsteps
       eps(i) = random_gaussian(VectorState) * noise_amp
       j = i + nsteps
       eps(j) = - dt * LOG(1.0_C_DOUBLE - random(VectorState))
       j = j + nsteps
       eps(j) = - dt * LOG(1.0_C_DOUBLE - random(VectorState))
       j = j + nsteps
       eps(j) = - four_dt * LOG(1.0_C_DOUBLE - random(VectorState))
    END DO
  END SUBROUTINE create_random_numbers
  
  !! Update the array eps of random numbers.
  !! For n_update instants, the four random numbers (Gaussian and exponential) are updated.
  SUBROUTINE update_random_numbers(VectorState, nsteps, dt, four_dt, noise_amp, n_update, eps)
    INTEGER(C_INT64_T), INTENT(INOUT) :: VectorState(:)
    INTEGER, INTENT(IN)               :: n_update, nsteps
    REAL(C_DOUBLE), INTENT(IN)        :: dt, four_dt, noise_amp
    REAL(C_DOUBLE), INTENT(INOUT)     :: eps(:)
    INTEGER                           :: chosen(nsteps), counter, trial_ind
    counter = 0
    chosen = 0
    DO WHILE (counter < n_update)
       ...
       IF (chosen(trial_ind) == 0) THEN
          chosen(trial_ind) = 1
          counter = counter + 1
          eps(trial_ind) = random_gaussian(VectorState) * noise_amp
          trial_ind = trial_ind + nsteps
          eps(trial_ind) = - dt * LOG(1.0_C_DOUBLE - random(VectorState))
          trial_ind = trial_ind + nsteps
          eps(trial_ind) = - dt * LOG(1.0_C_DOUBLE - random(VectorState))
          trial_ind = trial_ind + nsteps
          eps(trial_ind) = - four_dt * LOG(1.0_C_DOUBLE - random(VectorState))
       END IF
    END DO
  END SUBROUTINE update_random_numbers

  !! Read simulation parameters from input files provided on the command line.
  SUBROUTINE input(input_cnf, file_output, file_histo, file_final_state, dt, four_dt, T, &
                   noise_amp, xa, nsteps, ntrials, nsamples, n_update, kappa, x0, xmin, &
                   xmax, dx, in_SS)
    CHARACTER(256), INTENT(OUT)     :: input_cnf, file_output, file_final_state, file_histo
    INTEGER, INTENT(OUT)            :: nsteps, nsamples, n_update
    INTEGER(C_INT64_T), INTENT(OUT) :: ntrials
    LOGICAL, INTENT(OUT)            :: in_SS
    REAL(C_DOUBLE), INTENT(OUT)     :: dt, four_dt, T, noise_amp, xa
    ...
    DO
       ...
       IF (TRIM(param) == "center") READ(value_param(1:LEN_TRIM(value_param)),*) xa !! position for which we measure the local time
    END DO
10  CONTINUE
    CLOSE(UNIT = UNIT)
...
17  CONTINUE
    CLOSE(UNIT = UNIT)
    noise_amp = SQRT(2.0_C_DOUBLE * dt)
    T = dt * nsteps
    four_dt = 4.0_C_DOUBLE * dt
    ...
    END IF
    IF (xa < 0) THEN
       WRITE(*, "(a)") "The centre value for the local time is not provided."
       STOP
    END IF
  END SUBROUTINE input
  
  !! Read the list of all random numbers from a file.
  SUBROUTINE read_init_state(UNIT, nsteps, eps)
...
PROGRAM main
  ...
  REAL(C_DOUBLE)                  :: dt, four_dt, T, noise_amp, xa
  ...
  CALL input(file_init, file_output, file_histo, file_final_state, dt, four_dt, T, noise_amp, &
             xa, nsteps, ntrials, nsamples, n_update, kappa, x0, xmin, xmax, dx, in_SS)  
  ...
  IF (LEN_TRIM(file_init) > 0) THEN
     ...
     CALL read_init_state(unit_init, nsteps, eps)
     CLOSE(UNIT = unit_init)
  ELSE
     CALL create_random_numbers(VectorState, nsteps, dt, four_dt, noise_amp, eps)
  END IF
  ...
  x = compute_local_time(eps, nsteps, T, xa)
  ALLOCATE(eps_trial(SIZE(eps)))
  CALL CPU_TIME(start_time)
  DO i = 1_C_INT64_T, ntrials
     eps_trial = eps
     CALL update_random_numbers(VectorState, nsteps, dt, four_dt, noise_amp, n_update, eps_trial)
     trial_x = compute_local_time(eps_trial, nsteps, T, xa)
